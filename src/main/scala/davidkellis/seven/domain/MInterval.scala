...
import davidkellis.seven.Time.{compareDateTimes, intervalBetween}

object MInterval {
  def apply(intervals: Array[Interval]): MInterval = new MInterval(intervals)
  def empty: MInterval = MInterval(Array.empty[Interval])
}
